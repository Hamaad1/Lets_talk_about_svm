            ...
            if kernel_value in ['rbf', 'poly', 'sigmoid', 'linear']:
                for rep in range(repetitions):
                    if kernel_value == 'poly':
                        model = SVR(C=C, kernel=kernel_value, gamma=gamma, epsilon=epsilon, degree=degree, coef0=coef0, tol=tol, max_iter=max_iter)
                        ...
                        model = SVR(C=C, kernel=kernel_value, gamma=gamma, epsilon=epsilon, coef0=coef0, tol=tol, max_iter=max_iter)
                        model = MultiOutputRegressor(model, n_jobs=-1)
