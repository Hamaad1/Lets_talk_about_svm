...
def runSVR_C7(processed_data, base_name, results_directory,dataset_params, config):
    C = config['C']
                        ...
                        for degree_value in degree:
                            for coef0_value in coef0:
                                for rep in range(repetitions):
                                    if kernel_value in ['poly', 'sigmoid']:
                                        svr_lat = SVR(C=C, kernel=kernel_value, gamma=gamma_value, epsilon=epsilon, degree=degree_value, coef0=coef0_value, tol=tol_value, max_iter=max_iter)
                                        svr_lon = SVR(C=C, kernel=kernel_value, gamma=gamma_value, epsilon=epsilon, degree=degree_value, coef0=coef0_value, tol=tol_value, max_iter=max_iter)
                                        svr_alt = SVR(C=C, kernel=kernel_value, gamma=gamma_value, epsilon=epsilon, degree=degree_value, coef0=coef0_value, tol=tol_value, max_iter=max_iter)
                                       ...
                                       # Create the split folder first
                                        ...
                                        tol_folder = os.path.join(split_folder, f"C_{C}", f"kernel_{kernel_value}", f"gamma_{gamma_value}", f"tol_{tol_value}")
                                        if not os.path.exists(tol_folder):
                                            os.makedirs(tol_folder) 
                                        
                                        # Save the individual errors to CSV
                                        individual_errors_file = os.path.join(tol_folder , f"error_repetition{rep+1}.csv")
                                        ...
                                        predictions_file = os.path.join(tol_folder , f"predictions_repetition{rep+1}.csv")
                                        ...
                                        svr_lat = SVR(C=C, kernel=kernel_value, gamma=gamma_value,epsilon=epsilon, tol=tol_value, max_iter=max_iter)
                                        svr_lon = SVR(C=C, kernel=kernel_value, gamma=gamma_value,epsilon=epsilon, tol=tol_value, max_iter=max_iter)
                                        svr_alt = SVR(C=C, kernel=kernel_value, gamma=gamma_value,epsilon=epsilon, tol=tol_value, max_iter=max_iter)
    ...
    summary_list.append([base_name, C, best_kernel, best_degree if best_kernel == 'poly' else None, best_gamma, best_beta if best_kernel == 'sigmoid' else None, best_epsilon, best_split,best_tol, best_val_error, best_error])#, epsilon

    # Save the summary
    summary_df = pd.DataFrame(summary_list, columns=['Dataset', 'Best C', 'Best kernel', 'Best Degree', 'Best gamma', 'best Beta','Best Epsilon','Best Split', 'Tolerance',  'Error val', 'Testing error'])#
