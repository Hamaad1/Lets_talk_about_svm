            ...
            if kernel_value in ['rbf', 'poly', 'sigmoid', 'linear']:
                for gamma_value in gamma:
                    for rep in range(repetitions):
                        #print(f"Running C={C}, repetition={rep}, gamma={gamma_value}, Kernel={kernel_value}")
                        ...
                        if kernel_value in ['poly', 'sigmoid']:#== 'poly':
                            ...
                            # # Save predictions
                            # pred_df = pd.DataFrame(y_pred_testing, columns=['Latitude', 'Longitude', 'Altitude'])
                            ...
                            # errors_df = pd.DataFrame(np.round(testing_error, 2), columns=['3D Positioning Error'])
                            ...
                            # # Save results (same as your original setup)
                            # kernel_folder = os.path.join(results_directory, base_name, f'Kernel_{kernel_value}_Gamma_{gamma_value}')
                            # os.makedirs(kernel_folder, exist_ok=True)
                            # pd.DataFrame(y_pred, columns=['Latitude', 'Longitude', 'Altitude']).to_csv(os.path.join(kernel_folder, f'predictions_kernel_{kernel_value}_gamma_{gamma_value}.csv'), index=False)
                            # pd.DataFrame(np.round(test_error, 2), columns=['3D Positioning Error']).to_csv(os.path.join(kernel_folder, f'individual_errors_kernel_{kernel_value}_gamma_{gamma_value}.csv'), index=False)
