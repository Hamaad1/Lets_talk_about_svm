...
def runSVR_C2(processed_data, base_name, results_directory,dataset_params, config):#  C, kernel_values,epsilon, degree_values,  gamma, tol, max_iter):
        ...
        for kernel_value in tqdm(kernel, desc=f"Processing dataset: {base_name}, C:{C}, Epsilon: {epsilon}, gamma:{gamma}"):
            if kernel_value in ['rbf', 'poly', 'sigmoid', 'linear']:
                for rep in range(repetitions):
                    #print(f"Training with split {test_split}, repetition {rep + 1}/{repetitions}")
                    ...
                    if kernel_value in ['poly', 'sigmoid']:# == 'poly':
                        ...
                        for degree_value in degree:
                            for coef0_value in coef0:
                                ...
                                svr_lat = SVR(C=C, kernel=kernel_value, epsilon=epsilon, gamma=gamma, degree=degree_value, coef0=coef0_value, tol=tol, max_iter=max_iter)
                                svr_lon = SVR(C=C, kernel=kernel_value, epsilon=epsilon, gamma=gamma, degree=degree_value, coef0=coef0_value, tol=tol, max_iter=max_iter)
                                svr_alt = SVR(C=C, kernel=kernel_value, epsilon=epsilon, gamma=gamma, degree=degree_value, coef0=coef0_value, tol=tol, max_iter=max_iter)
